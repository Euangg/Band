...
const JIN_ZI_JUN = preload("uid://byhmitc7o2dx4")
...
func _ready() -> void:
	...
	t.start.connect(func():
		chen_ji_wei.show()
		chen_ji_wei.animation_player_pos.play("l")
		lin_yi_yan.show()
		lin_yi_yan.animation_player_pos.play("r")
		lin_yi_yan.animation_player_face.play("闭嘴")
...
func dia_笑死我了2():
	...
	t_next.start.connect(func():
		lin_yi_yan.show()
		lin_yi_yan.animation_player_face.play("苦笑")
...
func dia4():
	...
	t_next.start.connect(func():
		cheng_xiao_yi.show()
		cheng_xiao_yi.animation_player_face.play("闭嘴害羞")
		cheng_xiao_yi.animation_player_decoration.play("说话")
		)
	t_next.dialogue=[
		["大…大家好，我是节奏吉他。"+Global.name_player+"让我过来的。",0],
		]
	t_next.end.connect(func():
		cheng_xiao_yi.animation_player_decoration.play("RESET")
		dia4_1())
	ready_template(t_next)
...
func dia4_1():
	var t_next:Template=T_CHENG.instantiate()
	t_next.start.connect(func():
		#character_face_change_fade(node_cheng_xiao_yi,"捂嘴说话xx")
		cheng_xiao_yi.animation_player_face.play("捂嘴说话")
		)
	t_next.dialogue=[
		["我叫程晓依，是…男生……\n啊啊不过这不重要啦……也可以把我当成女生的……",0],
		]
	t_next.end.connect(func():
		var chen=cheng_xiao_yi
		chen.hide()
		dia5())
	ready_template(t_next)
...
func dia6():
	...
	t_next.start.connect(func():
		t_next.animation_player.play("r")
		lin_yi_yan.show()
		lin_yi_yan.animation_player_face.play("微笑"))
	t_next.dialogue=[
		["太好了，小荆，我们有节奏吉他了。",0]
		]
	t_next.end.connect(func():
...
func dia8():
	var t_next:Template=T_CHENG.instantiate()
	t_next.start.connect(func():cheng_xiao_yi.show())
	t_next.dialogue=[
		["好……不过我可能要先熟悉一下。",0]
		]
	t_next.end.connect(dia9)
	t_next.end.connect(func():cheng_xiao_yi.hide())
	ready_template(t_next)

func dia9():
	var t_next:Template=T_JING.instantiate()
	t_next.start.connect(func():jin_zi_jun.show())
	t_next.dialogue=[
		["那你自己看会吧。其他人先排。",0]
		]
	#t_next.end.connect(dia8)
	t_next.end.connect(func():jin_zi_jun.hide())
	ready_template(t_next)
